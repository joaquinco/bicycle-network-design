...
\begin{document}
  ...
  La formulación requiere la presencia de parámetros que en la práctica son de dificil o no son facilmente especificables en términos de un valor escalar. La cantidad y tipos de infrastructuras de ciclovía varían de país a país tanto en su disponibilidad como en sus costos, aún para el caso de Uruguay es dificil estimar, para las infrastructuras disponibles, sus costos de manera general ya que estos varían de la ubicación, congestión del área de la ciudad, precios de insumos como petroleo y mano de obra, reubicación de paradas de ómnibus y señales de tránsito, como se analiza en el analisis {\it Typical Costs of Cycling Interventions} (\ref{typicalcostsofcylcing}). En Bayá (2021) (\ref{inco2021}) se generaliza el cálculo de estos costos de manera que la infrastructura del nivel $i+1$ cuesta el doble que la $i$ siendo consistente con la evaluación de costos en el análisis antedicho.

  Los costos de usuarios de atravesar un arco varían en la práctica de factores como ancho de la vía, pavimento, volumen y velocidad del tránstio, entre otros factores, todo esto ponderado por el largo del arco. En el documento {\it Highway Capacity Manual} (\ref{hcm2010}) podemos encontrar el concepto de {\it Bycicle Level of Service} (BLOS) que agrupa estas consideracionen bajo un único valor y define seis niveles. Siguiendo el trabajo de (TODO: de donde sale esta formula?) se ha simplificado el manejo de los costos de usuario por infrastructura de ciclovía: tomando un valor $C_a$ sobre el el arco $a$ sin infrastructura (o con la infrastructura base), se puede computar el costo de usuario sobre la infrastructura $i \in {1,..,5}$ como ${C_a (28 - 3 (i + 1)) \over 25}$. Esto implica que se obtiene una proporción de $88\%, 76\%, 64\%, 52\% y 40\%$ sobre el costo de la infrastructura base para las infrastructuras 1 a 5 respectivamente.

  TODO: presupuesto? demandas? costo de usuario sobre infra base?
  ...
  Sobre el grafo y matriz de demanda antedichos, se agregan los siguientes parámetros base:
  ...
  \begin{description}
    ...
    \item[Presupuesto]: Se establece un presupuesto base del 30\% del costo de construir la infraestructuras: TODO: el factor del presupuesto es sobre que costo? Actualmente es sobre el costo total de construir la infrastructura mas barata en todo el grafo.
  \end{description}
  ...
  \begin{enumerate}
    ...
    \item{\label{hcm2010} Transportation Research Board, (2010). Highway Capacity Manual.}
    \item{\label{inco2021} Gabriel Bayá, Antonio Mauttone, Franco Robledo (2021). Bicycle network design considering discontinuities and different technological alternatives.}
  \end{enumerate}
\end{document}
