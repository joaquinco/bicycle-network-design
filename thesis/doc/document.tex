...
\begin{document}
  ...
  Esta formulación funcionó bien en el modelo final a los efectos de calcular la demanda transferida y ciertos flujos óptimos, pero sucedió que en algunos casos el modelo carecía de incentivos para seguir mejorando los costos de los flujos una vez pasado cierto punto de quiebre $Q_{kj}$, esto se detecto al observar que los valores de algunos flujos no eran unitarios y en que el costo del camino mas corto era igual al de algún punto de quiebre $Q_{kj}$. Para solucionar esto, se agregó otra variable, $r_{kj} \geq 0$ que modela la diferencia entre el punto de quiebre $Q_{kj}$ activo y el costo del camino mas corto $w_k$. Luego se agregó dicha variable a la función objetivo de manera que se incentivar el crecimiento de esta variable. La ecuación (\ref{eq:implfkoriginalineq}), que en el modelo final agrega los indices por par origen-destino $k$, quedó entonces de esta manera:
  ...
  La relajación de la integralidad de las variables $z_{kj}$, como se menciona en la sección de definición de $f_k$ debe seguir siendo estudiada ya que dependiendo el solver y la instancia los resultados pueden variar. Para algunos pares solver e instancia, la solución de la relajación es exactamente la misma mientras que en otros no. Las pruebas se hicieron con solvers GLPK y CBC (\footnote{Coin-OR Brunch and Cut solver}).
