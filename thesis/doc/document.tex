...
\begin{document}
  ...
  \begin{description}
    \item[Infrastructuras]: Se utilizan 2 infraestructuras además de la base.
  ...
  Se analizarán las perturbaciones sobre el conjunto de parámetros base aplicando una perturbación de algunos parámetros por vez, a contrario de una ejecución grid que puede terminar con muchisimas instancias debido a toda las combinaciones de parametros posibles.

  En primer lugar se perturbará el factor de presupuesto entre valores de 10\%, 60\% y 80\%, con el objetivo de observar si se cumple la simple intuición de que siempre a mayor presupuesto se obtienen mejores resultados.

  Luego, se analizará en mayor profundidad la forma de especificar la función $f$ junto a los puntos de quiebre y cómo influye la cantidad de infrastrucutas utilizadas. En particular, se alternará entre 5, 10, 20 y 50 puntos de quiebre, se definirán los puntos de manera que sean equidistantes en el codominio de $f$, es decir que si hay N puntos, de un punto al siguiente se obtenga una mejora de, aproximadamente, ${1 \over N - 1}$ en la proporción de demanda transferida. Se utilizarán las cuatro funciones de la figura (\ref{fig:fcatalog}). A su vez, se combinaran cantidades de infrastrucutas de 4, 5 y 6. El valor $m$ elegido se mantendrá fijo en el máximo posible, asumiendo 6 tipos de infrastructuras, independiente de la cantidad de infrastructuras con el objetivo de poder comparar efectivamente el efecto de disponer de más infrastructuras o no. Si computamos el valor $m$ en base a la cantidad de infrastructuras entonces obtendremos diferentes funciones. Esto tiene la contracara de que las instancias con menos de 6 tipos de infrastructuras disponibles no podrán transferir el 100\% de la demanda aunque dispongan de presupuesto infinito, esta observación no tiene efectos respecto a la comparativa.

  En total se tienen 51 instancias que seran ejecutadas con el solver CBC, con 10 threads y un tiempo máximo de 10 horas por instancia.
