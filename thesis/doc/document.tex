...
\begin{document}
  ...
  Durante la implementación de este modelo en un solver, se notó una falencia en la restricción (\ref{eq:breakpointsalt}): esta formulación apela a esta restricción para minimizar el $w_k$ de manera que se activen $Q_{kj}z_{kj}$ más bajos, el problema es que si $w_k$ es alto, por ejemplo, mayor a todos los puntos de quiebre definidos, entonces esta restricción se va a cumplir siempre con $z_{kj} = 1, \forall j \in J$ provocando que no haya incentivo para minimizar $w_k$. En la implementación, se modificó la restricción, agregando una variable de holgura $r_{kj}$:
  
  \begin{equation}
    Q_{kj} z_{kj} + r_{kj} = w_k,\;\forall k \in OD, j \in J
  \end{equation}

  Luego, dicha variable de holgura se agregó negada a la función objetivo de manera que penalice en los casos en que $w_k$ es mayor a $Q_{kj}$ e incentive cuando es menor, que es lo que se busca.
