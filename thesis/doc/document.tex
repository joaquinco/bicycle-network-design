...
\begin{document}
  ...
  Tomando como base el puntaje de BLOS estratificado de la tabla (\ref{table:blosscores}), podemos traducir de forma aproximada, los puntajes a proporción de mejora de un nivel a otro utilizando la función $C_{infra}(i) = {(28 - 3 (i + 1)) \over 25}$ donde $i \in I = \{0,1,2,\ldots\}$ es la infraestructura. Suponiendo que el nivel F de la escala de BLOS corresponde a infraestructura base (o infraestructura 0), la proporción de mejora sobre el costo base de las infraestructuras A-F correspondientes a los tipos de infrastructura 1-5 se pueden observar en la misma tabla.

  Dada una instancia del problema, llamamos $m$ a la proporción de mejora máxima que puede dar la mejor infraestructura. Incurrimos a la simplificación de considerar, al definir una instancia, la cantidad $T$ de tipos infraestructuras posibles a utilizar (y no cuáles), de manera que las infraestructuras utilizables sean $I = \{0,.., T - 1\}$, entonces el valor $m = C_{infra}(T - 1)$.
  ...
  % 17 mayo 2022: Por aca voy en las correcciones.
