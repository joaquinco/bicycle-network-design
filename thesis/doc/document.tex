...
\begin{document}
  ...
  \newpage
  \begin{abstract}
    Estudiamos el problema de atracción de demanda hacia la bicicleta desde otros medios de transporte bajo el fundamento de que la construcción de infraestructura de ciclovías es un aspecto clave para incentivar este comportamiento. Mayor incentivo en el transporte en bicicleta puede traer grandes beneficios tanto en la salud y bienestar de las personas como para la ciudades disminuyendo la congestión del transito, ayudando a la calidad del aire y contaminación sonora. Mediante la elección de dónde y qué tipo de infraestructura construir podemos influenciar las decisiones de la cantidad de demanda cubierta por la bicicleta y es así que planteamos un modelo de optimización que maximiza este valor en base a esas decisiones. Asumimos que la decisiones que toma la masa de usuarios, que viajan desde un origen a un destino, respecto a cuál medio de transporte utilizar se comporta conforme a una función que modela cuánto de la demanda utilizaría la bicicleta según el costo de usuario de trasladarse utilizando cierto estado de la red de ciclovías. Como novedad, proponemos la posibilidad de una transición paulatina de la demanda entre modos de transporte bajo el supuesto de que no todos los usuarios percibimos los costos de igual manera.
  ...
  Si bien la bicicleta puede utilizar la red de calles o incluso sendas peatonales como medio, no siempre estas son elegibles por los ciclistas para su uso. Factores como la cantidad y velocidad del tránsito automotor y ancho de la banquina o senda afectan considerablemente esta decisión. La necesidad de infraestructura especifica para la bicicleta es ampliamente reconocida (\ref{hunt2007}) y consideramos que políticas de inversión en infraestructuras de ciclovías es uno de los factores claves para incentivar su utilización.

  La planificación de construcción de ciclovías al igual que la transferencia de demanda, o lo que es lo mismo cubrimiento de la demanda, entre modos de transporte son problemas que han sido tratados de diferentes puntos de vista en la literatura. Diversos trabajos han trabajado sobre modelos que optimizan la demanda cubierta por cierto medio de transporte en función del costo de usuario. Este es el caso en (\ref{garcia2005}), (\ref{laporte2007}) y (\ref{marin2007}) donde el objetivo es maximizar la demanda cubierta para la planificación de la red de metro y trenes. El modelado de este comportamiento se hizo como {\it all-or-nothing}, de manera que si el costo usuario de ir de un origen a un destino es menor a un valor umbral predefinido entonces se considera que la demanda está cubierta.

  Luego existen diversos trabajos relacionado a la planificación de redes de ciclovía. En (\ref{mauttone2017}) se introduce un modelo MIP que planifica la red de ciclovías minimizando discontinuidades en estas. En este trabajo se considera un objetivo centrado en el usuario: minimizar el costo de los ciclistas asumiendo que estos se mueven por el camino más corto. Luego, en (\ref{baya2021}) se estudia un problema similar pero con el añadido de considerar más de un tipos de infraestructura de ciclovías lo que aumenta considerablemente la complejidad del modelo de optimización. En (\ref{lim2021}) se estudia la planificación de ciclovías teniendo en cuenta cubrimiento de demanda de dos formas orientado a la seguridad de los ciclistas: se busca minimizar la penalización total de caminos seguros que cubran la mayor cantidad de demanda posible. La primer lugar estudian la penalización en función de un valor que es la diferencia en la distancia entre los caminos seguros y los caminos más cortos potencialmente no seguro. Luego estudian la penalización de la pérdida de usuarios ciclistas en función del mismo valor descrito anteriormente. En este trabajo se define la función de penalización como función lineal de a partes mientras que el cubrimiento de la demanda se modela como {\it all-or-nothing} en el primer caso.
 ...
 \subsection{Hipótesis}
  ...
  Si bien dejamos sin definir explicitamente las funciones $f_k$ para facilitar el modelado debemos tener en cuenta que buscamos que la formulación completa sea lineal ya que buena parte de los avances en solvers se han desarrollado sobre este tipo de problemas.
  ...
  Si bien la formulación binivel expresa de buena manera lo que queremos resolver, en la práctica los problemas de programación lineal binivel, o BLPP de sus siglas en inglés, son en general de difícil resolución. Ya se ha demostrado en (\ref{bardbook}) que el BLPP lineal en su formulación general es NP-Hard. Además, los métodos existentes para su transformación a formulaciones de un nivel no son directos. En el apéndice (\ref{sec:kkttransform}) mencionamos la metodología más común pero con cierto costo analítico que consiste en sustituir el problema del segundo nivel por las condiciones de optimalidad de Karush-Kuhn-Tucker. En este trabajo tomamos un camino alternativo reescribiendo la formulación binivel como un nivel basado en sus características estructurales y en los valores que tienen sentido para sus parámetros. Previo a esta reestructura debemos adaptar la formulación binivel de manera que las variables de diferentes niveles no se encuentren en un producto, ya que esto deviene en restricciones no lineales luego de las transformaciones.
  ...
  Es deseable también que el mínimo de $f_k$ sea 0 y que el máximo sea la demanda total del par $k$: $D_k$, osea $f_k(\overline{W}) = 0$ y $f_k(\underline{W}) = D_k$. Es decir, si ninguna mejora es impuesta al camino más corto no haya transferencia de demanda asi como si la mejor infraestructura posible es construida toda la demanda se transfiera. Esto tiene sentido si trabajamos solamente con la demanda transferible a la bicicleta, dejando afuera la que ya utiliza dicho medio de transporte y la que en ningún caso la utilizaría.
  ...
  % TODO: terminar esta sección: comparar contra la situación actual de montevideo.

  \section{Conclusiones y trabajo a futuro}

  En este trabajo proponemos un nuevo enfoque para el problema de diseño de ciclovías. Consideramos varios tipos de infraestructuras y permitimos modelar el comportamiento de la demanda de manera que pueda adaptarse a diferentes curvas. Esta última característica es la principal diferencia con trabajos anteriores y puede el principal aporte a la comunidad académica.

  A futuro, se puede seguir trabajando sobre esta formulación para que considere otros aspectos tratados en otros trabajos como discontinuidades en los tipos de infraestructura (\ref{baya2021}), integrar el concepto de camino seguro, de (\ref{lim2021}), y permitir distinguir qué tipos de infraestructura se puede construir en cada arco de la red.

  También se podría adaptar el modelo a un enfoque de bicicletas compartidas, realidad que es común en otras partes del mundo.

  Respecto a los datos, en este trabajo estimamos y aproximamos muchos de los parámetros. Pensamos que puede ser interesante aplicar este problema con datos lo más realistas posibles. Luego, puede ser interesante comparar los resultados obtenidos con soluciones actuales implementadas en ciudades con buenos niveles de servicio de bicicletas que tengan información histórica de su utilización de manera de contrastar el modelo con una realidad.
