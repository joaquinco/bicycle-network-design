...
\begin{document}
  ...
  Los costos de usuarios de atravesar un arco varían en la práctica de factores como ancho de la vía, pavimento, volumen y velocidad del tránstio, entre otros factores, a su vez ponderado por el largo del arco. En el documento {\it Bicycle Level Of Service, Applied Model} (\ref{blos2007}) podemos encontrar el concepto de {\it Bycicle Level of Service} (BLOS), muy utilizado en la literatura, que agrupa estas consideracionen bajo un único valor. Para el cálculo del BLOS se tienen en cuenta los siguientes aspectos, que cuantificados en indicadores luego son utilizados en una fórmula para calcular el puntaje de BLOS que por simplificación se estratifican en 6 niveles del A al F.
  ...
  Para establecer una linea base sobre el comportamiento de las funciones de transferencia de demanda, que por simplificidad, asumimos que todas $f_k$ son iguales tomamos el trabajo de Performance evaluation of extreme bicycle scenarios (\ref{shwe2014}) que, analizando una extensiva encuesta, aproxima a grandes rasgos una relación lineal entre la proporción de viajes hechos en bicicleta y la cantidad de infrastructura de ciclovía per cápita. Siguiendo este razonamiento asumiremos la $f_k$ es lineal decreciente, en el sentido de que, a menor costo de usuario percibido a costa de más infrastructura construida para el par origen-destino $k$ resulta en mayor atracción de demanda o mayor proporción de viajes en bicicleta.
  ...
  TODO: presupuesto? costo de usuario sobre infra base?
