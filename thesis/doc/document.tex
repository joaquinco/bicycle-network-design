...
\begin{document}
  ...
  Luego del análisis hecho, ensamblaremos la formulación final de manera explícita en base a la que trabajaremos en lo que resta del trabajo. Nótese que podemos escribir la formulación binivel original a partir de esta fácilmente dada su similitud. Dado que tenemos dos variantes del modelado de las $f_k$ entonces también tendremos dos variantes de esta formulación final. Aquí se presenta solamente utilizando la versión 2 siendo la otra muy similar.
  ...
  En esta sección analizamos la formulación de un nivel (\ref{eq:objectivefinalalt})-(\ref{eq:respectinfraalt}) con algunas variantes y mostramos su validez de manera experimental.
  ...
  Sea $\beta > 0$, la función objetivo queda:
  ...
  La otra forma de abordar esta situación es agregar es agregar al objetivo el del problema de segundo nivel de la formulación (\ref{eq:objective1lvl})-(\ref{eq:flowbalance}). Dado que los dos objetivos optimizan en direcciones contrarias, el coeficiente del segundo objetivo debería ser un valor negativo. Sea $\rho > 0$, entonces podemos expresar el multiobjetivo como:
  ...
  Presentamos dos soluciones, una sin utilizar multiobjetivo, llamémosle S, y la otra, SM, utilizando cualquiera de las variantes multiobjetivo, ecuaciónes (\ref{eq:multipleobj1}) u (\ref{eq:multipleobj2}).
